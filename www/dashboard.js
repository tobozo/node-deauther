...
var setSatCache = function(data) {
    var satellites = data.satsVisible;
    if(satellites===undefined || !satellites.length) return;
    satellites.forEach(function(satellite) {
      var prn = satellite.prn;
      satCache[prn] = satellite;
      satCache[prn].time = Date.now();
    });
    satCache.satsActive = data.satsActive;
}
...
socket.on('nmeawifi', function(data) {
   data.added = Date.now();
   if(data.ssid===undefined) return;
   onWifiEvent({change:data});
   // expected packets look like these:
...
socket.on('wlan-message', function(data) {
  ...
  switch(data.nmea) {
    case 'nmea-wifi-enabled':
      $('.switch + span').text('NMEA');
    break;
    case 'nmea-wifi-disabled':
      $('.switch + span').text('');
    break;
  }
...
socket.on('state', function(state) {
    ...
    setSatCache(state); // update satellite cache, squash duplicates
    updateTable(state);
    updateMap(state);
});
...
function updateSatellite() {
    ...
    for(var idx in satCache) {
      if(idx=='satsActive') {
        data.satsActive = satCache[idx];
      } else {
        // Garbage collect
        if(now - satCache[idx].time > 30000) {
          delete(satCache[idx]);
        } else {
          data.satsVisible.push(satCache[idx]);
        }
      }
    }
    
    if(satelliteRendering) return;
    satelliteRendering = true;
    if(data.satsActive===undefined) return;
    
    rect = svg.selectAll("rect").data(data.satsVisible);
    text = svg.selectAll("text").data(data.satsVisible);
    
    if(data.satsVisible.length>50) console.log(data);
    
    $('.satellites').attr('data-amount', data.satsActive.length + '/' + data.satsVisible.length);
...
function updateMap(state) {
    ...
    if(state.fix=='2D' || state.fix=='3D') {
        ...
        if(state.hdop!== undefined && state.pdop!==undefined) {
            if(   state.hdop>50 || state.pdop>50 
               || 1>state.hdop  || 1>state.pdop) return;
...
function initializeMap() {
    ...
    $('#cmn-toggle-1').on('click', function() {
      if($(this).prop('checked')) {
        socket.emit("wlan-disable", true);
      } else {
        socket.emit("wlan-enable", true);
      }
    });
    ...
    setInterval(function() {
      ...
      if(fixVal>0) $fixProgress.val(fixVal-1);;
